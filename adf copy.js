...
var ADF = ADF || {
    ...
    Methods: {
        ...
        initAdf: function() {
            ...
            if (!!ADF.Methods.returnQueryValueByName('firstname')) {
                $('#personalFirstName').val(capitalize(ADF.Methods.returnQueryValueByName('firstname')));
            }

            if (!!ADF.Methods.returnQueryValueByName('lastname')) {
                $('#personalLastName').val(capitalize(ADF.Methods.returnQueryValueByName('lastname')));
            }

            if (!!ADF.Methods.returnQueryValueByName('zipcode')) {
                $('#personalZip').val(ADF.Methods.returnQueryValueByName('zipcode'));
            }
        ...
        fundCards: function() {
            ...
            $('#giftSummary').on('blur', '.fund-card input', function() {
                if (!isNaN($(this).val())) {
                    if (Number($(this).val()) < 5.00) {
                        $(this).val('0.00');
                        if ($(this).next('.min-amount').length === 0) {
                            $(this).parent().append('<p class="min-amount">Please enter a minimum of $5.</p>');
                        }
                    } else {
                        if ($(this).next('.min-amount').length !== 0) {
        ...
        getDonationData: function() {
            ...
            var donation = {
                ...
                MerchantAccountId: "864426b2-20a0-43aa-95f6-c850d757b026", // ADF.Defaults.MerchantAccountId,
        ...
        giftOptions: function() {
            ...
            var onetimeGift = $('#onetimeGift'),
                ...
                // pledgeGiftStartDate = $('#pledgeStartDate'),
            ...
            // pledgeGiftStartDate.on('change', function() {
            //     // var newPledgeStartDate = new Date($(this).val().replace(/-/g, '\/')),
            //     //     newPledge = newPledgeStartDate.toLocaleDateString('en-US', {
            //     //         month: '2-digit',
            //     //         day: '2-digit',
            //     //         year: 'numeric'
            //     //     }).replace(/\u200E/g, '');
            ...
            //     // console.log(myFutureDate.setDate(myFutureDate.getDate() + 8));
            //     $('#pledgeStartDate').attr('data-date', $(this).val());
            //     // $('#pledgeStartDate').attr('data-date', newPledge);
        ...
        datePicker: function() {
            ...
            if ($("#pledgeStartDate").length !== 0) {
                ...
                $("#pledgeStartDate").datepicker({
                    minDate: getMinDate(),
                });

                $("#pledgeEndDate").datepicker({
                    dateFormat: "mm/dd/yyyy"
                });
                $("#pledgeStartDate").datepicker("setDate", getMinDate()).attr('data-date', getMinDate());
                // $("#pledgeEndDate").datepicker("setDate", getMinDate()).attr('data-date', getMinDate());
        ...
        pledgeSummary: function() {
            ...
            if ($('.fund-card.empty').hasClass('hidden') && $('#pledgeFrequency').val() !== '-1' && $('#pledgeInstallments').val() !== '' && $('.min-amount').length === 0) {
                ...
                //         year: 'numeric'
                // })
                var GiftLastPaymentDate = donationService.getRecurringGiftLastPaymentDate(numberOfInstallments, frequencyCode, installmentStartDate, installmentMonth, installmentDayOfMonth),
